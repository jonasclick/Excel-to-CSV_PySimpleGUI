...
def settings_window(settings):
    # – – – – – – GUI Definition – – – – – – #
    layout = [[sg.Text("Adjust your preferred behaviour for the CSV file.")],
              [sg.Text("Separator"), sg.Input(settings["CSV"]["separator"], s=1, key="-SEPARATOR-"),
               sg.Text("Decimal"), sg.Combo(settings["CSV"]["decimal"].split("|"), default_value=settings["CSV"]["decimal_default"], s=1, key="-DECIMAL-"),
                sg.Text("Sheet Name"), sg.Input(settings["EXCEL"]["sheet_name"], s=20, key="-SHEET_NAME-")],
                [sg.Button("Save Settings", s=16)]]
    window = sg.Window("Settings Window", layout, modal=True) #modal: blocks all other windows of the app until you close this one.
    while True:
...
def main_window():
    ...
    while True:
        ...
        if event == "About":
            window.disappear()
            sg.popup(window_title, "Version 1.0", "This app was written by Jonas Vetsch, closely following a tutorial by 'Coding Is Fun'.", "Tutorial link: https://www.youtube.com/watch?v=LzCfNanQ_9c", grab_anywhere=True, modal=True) #when I try to exit out of the popup it seems to freeze until I come back to the popup.
            window.reappear()
