...
def main_window():
    ...
    layout = [[sg.MenubarCustom(menu_def, tearoff=False)], #tearoff=true lets you drag the extended menu list around like a window
        [sg.Text("Select Input File"), sg.Input(key="-IN-"), sg.FileBrowse(file_types=(("Excel Files", "*.xlsx"),))], #key is the key assigned in the dictionary created from the input
        [sg.Text("Select Output Folder"), sg.Input(key="-OUT-"), sg.FolderBrowse()],
        [sg.Button("Display Excel File"), sg.Button("Convert To CSV")],
